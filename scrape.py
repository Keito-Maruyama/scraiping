...
def extract_data(soup):
    ...
    data['course_code'] = soup.select_one('#unified-course-items-180353 > div > div > div.catalog-search-result-table-row.catalog-table-row-in-mylist-2024-51325 > div.catalog-search-result-table-cell.code-cell > div:nth-child(2)').text.strip() if soup.select_one('#unified-course-items-180353 > div > div > div.catalog-search-result-table-row.catalog-table-row-in-mylist-2024-51325 > div.catalog-search-result-table-cell.code-cell > div:nth-child(2)') else "授業コード"
    data['department_id'] = "所属学部・学科のID"  # 所属学部・学科の ID は取得できないので、"-" で代用
    
    semester_element = soup.select_one('#catalog-main-container > div.catalog-page-detail-scrollable-container.catalog-horizontal-scrollable-container > div > div > div.catalog-page-detail-table-row.catalog-table-row-in-mylist-2024-51325 > div.catalog-row > div.catalog-page-detail-table-cell.semester-cell')
    data['semester'] = semester_element.text.strip() if semester_element else "学期"

    data['genre'] = "授業ジャンル"  # 授業ジャンルの情報は取得できないので、"" で代用
    data['description'] = soup.select_one('#catalog-main-container > div.catalog-page-common-container > div.catalog-page-detail-lecture-aim') else "授業説明"
    data['objectives'] = "目標"  # 授業目標の情報は取得できないので、"" で代用
    data['content'] = soup.select_one('#catalog-main-container > div.catalog-page-detail-card-container > div > div:nth-child(1) > div.catalog-page-detail-card-body > div').text.strip() if soup.select_one('#catalog-main-container > div.catalog-page-detail-card-container > div > div:nth-child(1) > div.catalog-page-detail-card-body > div') else "内容"    
    data['instructor_id'] = "教授id"  # 教授の ID は取得できないので、"-" で代用
    data['grading_criteria'] = soup.select_one('#catalog-main-container > div.catalog-page-detail-card-container > div > div:nth-child(3) > div.catalog-page-detail-card-body > div') else "評価基準"      
